...
function changeSelectedRow(id){
    if(selectedId)
        document.getElementById('typeId:' + selectedId).classList.remove('selected-row');
...
updateBtn.addEventListener('click', () => {
	if(selectedId){
		...
		document.getElementById('typeModalTitle').textContent = 'Modificar Tipo de Servicio';
		
		const description = document.getElementById('typeId:' + selectedId).getElementsByTagName('td')[0].textContent;
