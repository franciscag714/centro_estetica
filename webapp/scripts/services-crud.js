...
function changeSelectedRow(id){
    if(selectedId)
        document.getElementById('serviceId:' + selectedId).classList.remove('selected-row');
...
updateBtn.addEventListener('click', () => {
	if(selectedId){
		...
		document.getElementById('serviceModalTitle').textContent = 'Modificar Servicio';
		
		const celdas = document.getElementById('serviceId:' + selectedId).getElementsByTagName('td');
		
		serviceModal.querySelector("[name='description']").value = celdas[0].textContent;
		serviceModal.querySelector("[name='type']").value = celdas[1].dataset.typeid;
